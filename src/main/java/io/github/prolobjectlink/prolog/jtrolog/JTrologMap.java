 ...
 * @since 1.1
 */
public final class JTrologMap extends AbstractCompounds implements PrologMap {

	private Map<PrologTerm, PrologTerm> map;

	JTrologMap(PrologProvider provider, int size) {
		super(PrologTermType.MAP_TYPE, provider);
		map = new LinkedHashMap<PrologTerm, PrologTerm>(size);
	}

	JTrologMap(PrologProvider provider, Map<? extends PrologTerm, ? extends PrologTerm> m) {
		this(provider);
		putAll(m);
	}

	JTrologMap(PrologProvider provider) {
		this(provider, 16);
	}
	...
	public PrologTerm[] getArguments() {
		...
		for (int j = 0; j < args.length && i.hasNext(); j++) {
			...
			args[j] = new JTrologEntry(p, e.getKey(), e.getValue());
		}
		return args;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		JTrologMap other = (JTrologMap) obj;
		if (map == null) {
			if (other.map != null)
				return false;
	...
	public PrologTerm getTail() {
		JTrologMap m = new JTrologMap(provider, map);
	...
	private class PrologMapIterator extends AbstractIterator<PrologTerm> implements Iterator<PrologTerm> {
		...
		private PrologMapIterator() {
			...
			for (Iterator<Entry<PrologTerm, PrologTerm>> i = map.entrySet().iterator(); i.hasNext();) {
				Entry<PrologTerm, PrologTerm> e = i.next();
				PrologTerm t = new JTrologEntry(provider, e.getKey(), e.getValue());
				set.add(t);
			}
			itr = set.iterator();
		}
