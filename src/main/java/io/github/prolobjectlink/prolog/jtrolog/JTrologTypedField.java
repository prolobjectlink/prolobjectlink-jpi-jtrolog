...
public class JTrologTypedField extends JTrologField implements PrologTypedField {

	private final PrologTerm kind;

	JTrologTypedField(PrologProvider provider, PrologTerm name, PrologTerm kind) {
		super(provider, name);
		this.kind = kind;
	}

	JTrologTypedField(PrologProvider provider, String kind, int position) {
		super(provider, provider.newVariable(position));
		this.kind = provider.newVariable(kind, position);
	}

	JTrologTypedField(PrologProvider provider, String name, String kind, int position) {
